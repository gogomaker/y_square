...
module address_decoder(
  ...
  wire is_mem_range = |address[15:5];

  // 2. 장치별 상세 디코딩 (비트 4와 3을 이용한 MUX 구조)
  // 00xxx (0x00~0x07): Digital In
  assign din_en     = mem_access & !is_mem_range & (address[4:3] == 2'b00);
  
  // 01xxx (0x08~0x0F): Digital Out
  assign dout_en    = mem_access & !is_mem_range & (address[4:3] == 2'b01);
  
  // 10xxx (0x10~0x17): UART RX
  assign uart_rx_en = mem_access & !is_mem_range & (address[4:3] == 2'b10);
  
  // 11xxx (0x18~0x1F): UART TX
  assign uart_tx_en = mem_access & !is_mem_range & (address[4:3] == 2'b11);
